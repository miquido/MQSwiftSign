...
internal struct ConfigurationListObject {
	private(set) var defaultConfigurationName: String?
	private(set) var buildConfigurationRefs: [String]?

	init(properties: Dictionary<String, Any>) {
		self.defaultConfigurationName = properties["defaultConfigurationName"] as? String
		self.buildConfigurationRefs = properties["buildConfigurations"] as? Array<String>
	}
}

internal struct ConfigurationObject {
	var name: String?
	var buildSettings: ConfigurationObjectBuildSettings?

	init(properties: Dictionary<String, Any>) {
		self.name = properties["name"] as? String
		self.buildSettings = (properties["buildSettings"] as? Dictionary<String, Any>)
			.map({
				.init(properties: $0)
			})
	}
